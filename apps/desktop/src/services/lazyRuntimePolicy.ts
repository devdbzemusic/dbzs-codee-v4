...
import type { RuntimeSlotId, RuntimeSlotStatus, RuntimeStatus } from "@dbzs/shared";
 ...
 *
 * HTTP-reachable is not the same as inference-ready: when the status carries
 * a `chat_ready` flag (RuntimeSlotStatus), that flag — not just `state ===
 * "running"` — decides readiness, since a process can be running while the
 * model is still loading/warming up.
 */
export function isWorkModelLoaded(status: RuntimeStatus | RuntimeSlotStatus | null | undefined): boolean {
  if (!status || status.state !== "running") return false;
  if (looksLikeOrchestratorModel(status)) return false;
  if ("chat_ready" in status) {
