export type StandaloneView = "settings" | "runtime-chat" | "platform-diagnostics" | "splash" | null;
...
export function buildSplashWindowUrl(rendererUrl: string): string {
  const base = rendererUrl.split("#")[0]?.split("?")[0] ?? rendererUrl;
  const normalizedBase = base.endsWith("/") ? base.slice(0, -1) : base;
  return `${normalizedBase}/?view=splash#splash`;
}
