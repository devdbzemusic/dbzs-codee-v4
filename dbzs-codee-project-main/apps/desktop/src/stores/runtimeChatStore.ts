...
import { loadToolProfile, saveToolProfile, shouldUseAgentTurnLoop, isTrivialConversationMessage } from "@/services/runtimeChatAgentConfig";
...
import { classifyTaskTypeDetailed, brokerDecision, formatModelDisplayLabel, BindingModelError } from "@/services/modelSelectionBroker";
import {
  answeredFieldIds,
...
function mapBrokerAgentToShared(agent: import("@/services/modelSelectionBroker").ModelTargetAgent): ModelTargetAgent {
  switch (agent) {
    case "planner":
      return "planner";
    case "coder":
...
function mapWorkflowAgentToShared(
 ...
 */
function isClarificationFieldBlockedInMessages(
  messages: RuntimeChatMessage[],
  workspaceId: string,
...
export const useRuntimeChatStore = create<RuntimeChatState>((set, get) => ({
  ...
  sendMessage: async (
    ...
    let skillIds = sendOptions?.enabledSkillIds ?? get().enabledSkillIds;
    let effectiveAgent = resolveSkillTargetAgent(skillIds, targetAgent);
    let activeSkillContext: ActiveSkillRuntimeContext | null = null;
    const toolsEnabled = sendOptions?.toolsEnabled ?? get().toolsEnabled;
    const jobHint = get().consumeJobContextHint();
    const resolvedContextHint = sendOptions?.contextHint ?? contextHint ?? jobHint ?? null;
    const isAutoTrivial = sendOptions?.agentMode === "auto" && isTrivialConversationMessage(trimmedContent);
    const includeWorkspaceContext = sendOptions?.includeWorkspaceContext ?? true;
    ...
    const sticky = sendOptions?.stickyTaskType;
    if (
      sticky &&
      sticky !== "casual_chat" &&
      sticky !== "normal_chat" &&
    ...
    let workflowAssignment = resolveCanonicalWorkflowAssignment({
    ...
    if (metaIntent === "summarize_active_task") {
      const summaryText = buildDeterministicActiveTaskSummary({
        ...
        lastRunOutcome: get().lastActivity?.summary ?? get().activeRun?.status ?? null,
        warmupDetail:
          get().lastRouting?.warmupStatus === "failed"
            ? "warmup_failed"
            : get().lastRouting?.warmupStatus ?? null
      });
      set((state) => ({
        messages: [
          ...state.messages,
          { id: `msg-${Date.now().toString(36)}-user-summary`, role: "user", content: trimmedContent },
          {
    ...
    if (continuation.needsAmbiguityAsk && activeContract && workspaceRootForWorkflow) {
      ...
      if (
        isClarificationFieldBlockedInMessages(get().messages, workspaceId, "workflow_scope_decision") ||
      ...
      set((state) => ({
        messages: [
          ...
          { id: messageId, role: "system", content: ambiguity.prompt, actions: [action] }
        ]
      }));
      return false;
    }
    ...
    if (isReviewRemediation) {
      ...
      const askRemediationQuestion = (question: AssistantQuestion): false => {
        const runId = `run-${Date.now().toString(36)}-${Math.random().toString(36).substring(2, 6)}`;
      ...
      let deterministicRemediation: Awaited<
        ReturnType<typeof loadReviewFindingsForRemediation>
      >;
      try {
      ...
      try {
        findingsFile = await reviewApi.openReviewArtifact?.(
          remediationWorkspaceRoot,
          review.reviewId,
          "findings"
        );
      } catch (error) {
      ...
      const scope =
        selection.scope === "selected" ? "individual" : selection.scope;
    ...
    if (
      ...
      const reviewRequest = buildRepositoryReviewRequest({
        ...
        selectedPaths: activeFile?.path ? [activeFile.path] : undefined,
      ...
      const topFindings = reviewResult.findings
        .filter((finding) => !finding.needsVerification)
        .slice(0, 3);
      const summaryLines = [
        ...
        ...(topFindings.length
          ? ["", "Top Findings:", ...topFindings.map((finding, index) =>
      ...
      const success =
        ...
        reviewResult.outcome === "completed_with_warnings" ||
        reviewResult.outcome === "degraded_heuristic_only";
    ...
    try {
      ...
      if (slotValidationEnabled && contextSlotId && routing.modelId) {
        ...
        try {
          ...
          if (
            sendOptions?.forceUseResidentModel &&
            currentSlotStatus?.model_id &&
            runtimeSlotManager.isSlotReady(currentSlotStatus)
          ) {
            modelToStart = currentSlotStatus.model_id;
          ...
          const warmup = await runtimeSlotManager.warmupInference(
            slotId,
            modelToStart,
            timeoutManager.getWarmup(),
            runAbortController.signal,
            brokerDecisionFull?.decisionId
          );
          assertStartStillValid();
          if (!warmup.ok) {
            ...
            throw new BindingModelError(
              `Das konfigurierte ${roleLabel}-Modell konnte nicht inferenzbereit gestartet werden.${warmup.detail ? ` (${warmup.detail})` : ""}`,
              warmup.error === "runtime_oom" ? "runtime_oom" : "warmup_failed",
              [
          ...
          updateActiveRun((r) => ({
            ...appendRunEvent(r, "runtime.check.completed", "runtime_inference_ready", {
              slotId,
              preview: warmup.detail,
              readinessStage: "inference_ready",
              tokenVerified: true
            }),
            warmupStatus: "ready",
            readinessStage: "inference_ready",
            warmupDiagnostics: warmup.diagnostics
          }));
