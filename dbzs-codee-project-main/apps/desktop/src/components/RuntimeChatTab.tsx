...
export function RuntimeChatTab({
  ...
  const statusLabel = useMemo(() => {
    const loading =
      ...
      Boolean(
        activeRun &&
          !activeRun.firstTokenAt &&
          activeRun.events.some((event) => event.type === "runtime.check.started")
      );
    return formatLazyRuntimeStatusLabel({
      backendReachable: runtimeReady,
      workModelLoaded: workModelReady && Boolean(lastRouting?.modelId),
      lastRoutingModelName: lastRouting?.modelName,
      lastRoutingModelId: lastRouting?.modelId,
      lastRoutingAgentLabel: lastRouting ? agentLabel(lastRouting.targetAgent) : null,
      lastRoutingReasons: lastBrokerDecision?.reason ?? null,
      activeRunLoading: loading,
      loadingRoleHint: "Arbeitsmodell wird bei Bedarf geladen …"
    });
  }, [activeRun, isSending, lastBrokerDecision?.reason, lastRouting, runtimeReady, workModelReady]);
