...
ipcMain.handle("dbzs:backend:reload", async () => {
  ...
  void maybeAutostartOrchestratorRuntime();
...
async function maybeAutostartOrchestratorRuntime(): Promise<void> {
  try {
    const settings = await requestBackend<AppSettings>("/settings");
    if (settings.autoStartOrchestratorRuntime !== true) {
      return;
    }
    const modelId = settings.defaultOrchestratorModelId?.trim();
    if (!modelId) {
      console.info("[runtime-cleanup] Skipping orchestrator autostart: no defaultOrchestratorModelId configured.");
      return;
    }
    await requestBackend("/runtime/slots/orchestrator_cpu/start", {
      method: "POST",
      body: JSON.stringify({ model_id: modelId })
    });
    console.info(`[runtime-cleanup] Orchestrator autostart requested after backend readiness: ${modelId}`);
  } catch (error) {
    console.info("[runtime-cleanup] Orchestrator autostart skipped:", error);
  }
}
...
function createWindow(): void {
  ...
  configureDbzsWindowSecurity(mainWindow);
...
app.whenReady().then(async () => {
  ...
  configureAppMenu();
  createWindow();
  void startup.ensureStarted({ waitUntilReady: false });
  void startup.ensureStarted({ waitUntilReady: true, timeoutMs: 60_000 }).then((status) => {
    if (status.state === "ready") {
      return maybeAutostartOrchestratorRuntime();
    }
    return undefined;
  });
