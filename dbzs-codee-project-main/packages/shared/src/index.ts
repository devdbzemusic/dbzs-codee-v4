...
export interface SettingsDiagnostics {
  ...
  /** Windows runtime root (default D:/win_runtimes). */
  winRuntimesDir?: string;
}

export interface AppSettings {
  /** Persistence schema version (settings.json). */
  schemaVersion?: number;
  /** Monotonic revision for optimistic PATCH concurrency. */
  revision?: number;
  ...
  /** Minutes of idle before unloading non-orchestrator work slots (0 = disabled). */
  ...
  /** Stream-Idle-Timeout in Sekunden (kein Token). */
  timeoutStreamIdleSeconds?: number;
  /** Wartezeit bis zum ersten Token in Sekunden. */
  timeoutFirstTokenSeconds?: number;
  /** Maximale Generierungsdauer in Sekunden. */
  timeoutGenerationSeconds?: number;
  /** Prompt-Eval-Timeout in Sekunden (vor erstem Token). */
  timeoutPromptEvalSeconds?: number;
  /** Stream-Idle bei CPU-Safe / gpuLayers=0 (Sekunden). */
  timeoutCpuSafeStreamIdleSeconds?: number;
  /** First-Token bei CPU-Safe / gpuLayers=0 (Sekunden). */
  timeoutCpuSafeFirstTokenSeconds?: number;
  /** Generation bei CPU-Safe / gpuLayers=0 (Sekunden). */
...
export const DEFAULT_WEB_RESEARCH_POLICY: WebResearchPolicy = {
  ...
  maxResults: 20,
  maxFetchesPerTurn: 5,
  maxBytesPerDocument: 1024 * 1024, // 1 MB
  maxTotalBytesPerTurn: 4 * 1024 * 1024, // 4 MB
  defaultTimeoutMs: 36000, // 360 s
  allowHttp: true,
