...
def migration_001_add_legacy_timestamp_column(conn: sqlite3.Connection) -> None:
    ...
    `agent_logs` only ever had a `timestamp` column on installs old enough
    to predate AgentRegistryService's own `created_at` rename (see
    app/agents/service.py's `_init_db()`, which already handles that
    rename/backfill directly). A fresh install's `agent_logs` table never
    has a `timestamp` column at all — backfilling from it unconditionally
    made this migration crash with "no such column: timestamp" the first
    time it was ever actually run (this code path was previously dead;
    wiring it into the boot sequence is what surfaced the bug).
    """
    conn.execute("""
    ...
    has_timestamp_column = any(
        row[1] == "timestamp" for row in conn.execute("PRAGMA table_info(agent_logs)").fetchall()
    )
