...
def parse_llama_server_sse_line(line: str) -> str | None:
    ...
    # Reasoning models (e.g. Qwen3.5-class) stream their output via
    # `reasoning_content` instead of (or before) `content`. Dropping it here
    # made an actively-generating model look like it produced no token at
    # all — surface it the same way so streaming/warm-up both see real output.
...
def parse_llama_server_sse_has_tool_call(line: str) -> bool:
    """True when this SSE chunk carries a `tool_calls` delta.

    Used by warm-up verification alongside `parse_llama_server_sse_line` —
    a tool-call-only response is real model output too, not "no token".
    """
    stripped = line.strip()
    if not stripped.startswith("data:"):
        return False
