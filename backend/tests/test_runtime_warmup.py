...
def test_warmup_slot_inference_ready(monkeypatch) -> None:
    ...
    service._slots = {}
    service._shared_slot_bindings = {}
    service._external_slot_pids = {}
    service._statuses = {
...
def test_warmup_slot_inference_ready_via_reasoning_content(monkeypatch) -> None:
    """A reasoning model (e.g. Qwen3.5-class) streams via `reasoning_content`
    instead of `content` — this must count as real output, not
    warmup_empty_response (DBZS_CODEE_V4_POST_REPAIR_ANALYSIS.md §1)."""
    service = RuntimeService.__new__(RuntimeService)
    service._simulation_mode = False
    service._slots = {}
    service._shared_slot_bindings = {}
...
def test_warmup_slot_inference_ready_via_tool_call(monkeypatch) -> None:
    """A tool-call-only warm-up response is real model output too, not
    "no token"."""
    service = RuntimeService.__new__(RuntimeService)
    service._simulation_mode = False
    service._slots = {}
    ...
    sse = b"".join(
        [
            b'data: {"choices":[{"delta":{"tool_calls":[{"id":"1","function":{"name":"ok"}}]},"finish_reason":null}]}\n',
    ...
    assert result.outcome == "inference_ready"
    assert result.diagnostics is not None
    assert result.diagnostics.tool_call_detected is True


def test_warmup_empty_response_carries_raw_preview(monkeypatch) -> None:
    """warmup_empty_response must attach the raw response body so a
    parser/model-shape mismatch is diagnosable, not just "no token"."""
    service = RuntimeService.__new__(RuntimeService)
    service._simulation_mode = False
    service._slots = {}
    service._shared_slot_bindings = {}
    ...
    def fake_urlopen(request_obj, timeout=None):
        ...
        if payload.get("stream"):
            return FakeResponse(b'data: {"choices":[{"delta":{"unexpected_field":"OK"}}]}\ndata: [DONE]\n')
        return FakeResponse(
            json.dumps({"choices": [{"message": {"content": ""}, "finish_reason": "stop"}]}).encode("utf-8"),
    ...
    assert result.diagnostics is not None
    assert result.diagnostics.raw_response_preview is not None
    assert "unexpected_field" in result.diagnostics.raw_response_preview
