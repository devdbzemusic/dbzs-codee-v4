...
async def test_lifespan_cleanup_success():
    ...
    # Patch all required imports/calls inside the lifespan handler. Startup
    # tasks (_run_startup_tasks) are patched out too — this test only cares
    # about shutdown cleanup, and without this the real database/model-index/
    # resident-model boot tasks would run against the developer's actual
    # app-data dir (and could even spawn a real llama.cpp process if their
    # persisted settings have autostart enabled).
    with patch("app.api.model_profiles._multi_server_manager", mock_multi_server_manager), \
         ...
         patch("app.main._run_startup_tasks", new=AsyncMock()), \
